...
function logo(x,y,w){return `<g transform="translate(${x} ${y})"><rect width="${w}" height="${w*.22}" rx="4" fill="#fff" opacity=".96"/><text x="${w*.05}" y="${w*.145}" font-family="Arial, sans-serif" font-size="${w*.08}" font-weight="800" fill="${BLUE}">Deutscher Fenstershop</text></g>`}
function drutex(x,y,w){return `<g transform="translate(${x} ${y})"><rect width="${w}" height="${w*.28}" rx="4" fill="#fff" opacity=".94"/><text x="${w*.09}" y="${w*.19}" font-family="Arial, sans-serif" font-size="${w*.13}" font-weight="900" fill="${BLUE}">DRUTEX</text></g>`}
