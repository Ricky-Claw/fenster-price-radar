...
- **API:** `POST /api/chatbot` (`api/chatbot.js`) · **Widget:** `public/chatbot-widget.js` · **Logik:** `src/chatbot/fenstershopChatbot.js` · **Regelwerk:** `programmierlogik_chatbot_final_mit_anfrage_status.md` + `FENSTERSHOP_CHATBOT_PLAN.md`.
- **LLM-Kette (in dieser Reihenfolge):** (1) **Codex-Brücke** — `src/chatbot/codexBridgeClient.js` ruft den VPS-Dienst `tools/janela-bridge/`, der die dort angemeldete Codex-CLI nutzt (Abo-Kontingent statt API-Abrechnung); aktiv nur mit `JANELA_BRIDGE_URL` + `JANELA_BRIDGE_TOKEN`, Adresse muss `https` und `.hstgr.cloud` sein. (2) **GPT-5.6 Luna** über `OPENAI_API_KEY`/`OPENAI_OAUTH_TOKEN`. (3) **Claude Haiku 4.5**. Ohne Zugang bleibt der Regel-Entwurf ohne Politur. Welcher Anbieter tatsächlich bedient: `npm run check:llm` (mit `-- --gegen-produktion` gegen die Live-Seite).
- **Sensible Nachrichten** (Bestell-/Rechnungsnummer, IBAN, Anschrift, Rufnummer, E-Mail) lösen **keinen** LLM-Aufruf aus (`mustSkipLlmForPrivacy`) — sie verlassen das Haus nicht. `LLM_SAFE_INTENTS` nie erweitern, um einen Test grün zu bekommen: harte Regelantworten dürfen kein Modell umformulieren.
