...
Vergleicht **eigene Brutto-Listenpreise** gegen **Fensterblick** und **Fensterversand**. Login-gated (`middleware.js` + `api/login.js`/`api/logout.js`, HMAC-Cookie `fenster_radar_session`).
- **Datenquelle:** `data/comparison-catalog.json` steuert die In-Repo-Provider-Skripte unter `src/providers/{dfs,fensterblick,fensterversand}/`. Diese scrapen die Live-Konfigurator-JSON-APIs der Wettbewerber und schreiben Laufdaten nach `results/` (gitignored).
...
- **Weekly-Update:** Primärer Trigger ist ein VPS-Cron auf Hostinger `nexus-host`. Die GitHub-Action `weekly-price-update.yml` bleibt `workflow_dispatch` als manueller Fallback. Ablauf: `npm run prices:update` (Live-Scrape aller drei Provider + Sync + `verify:prices`) → `npm run build` → Commit der `public/data`-Artefakte.
